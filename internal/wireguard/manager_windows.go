...
func executable() string {
	candidates := []string{`C:\Program Files\WireGuard\wireguard.exe`, `C:\Program Files (x86)\WireGuard\wireguard.exe`, `wireguard.exe`}
...
func InstallAndStart(repoRoot string, node model.Node) error {
	...
	if status := Status(node); status.Connected { return nil }
...
func Stop(node model.Node) error {
	...
	if err != nil {
		if !strings.Contains(strings.ToUpper(string(out)), "STOPPED") { return fmt.Errorf("stop tunnel: %s", strings.TrimSpace(string(out))) }
	}
	return nil
}
...
func Uninstall(node model.Node) error {
	...
	if err != nil { return fmt.Errorf("uninstall tunnel: %s", strings.TrimSpace(string(out))) }
	return nil
}

func serviceExists(name string) bool { return exec.Command("sc.exe", "query", name).Run() == nil }

func Status(node model.Node) model.Status {
	if _, err := exec.LookPath("sc.exe"); err != nil { return model.Status{Installed:false, Message:"Windows service tooling unavailable"} }
	if !serviceExists(serviceName(node)) { return model.Status{Installed:false, Tunnel:serviceName(node), Message:"Tunnel not installed"} }
	out, err := exec.Command("sc.exe", "query", serviceName(node)).CombinedOutput()
	if err != nil { return model.Status{Installed:true, Tunnel:serviceName(node), Message:string(out)} }
	connected := strings.Contains(strings.ToUpper(string(out)), "RUNNING")
	message := "Stopped"; if connected { message = "Connected" }

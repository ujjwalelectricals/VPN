...
func TestLoadAndFind(t *testing.T) {
	...
	if err := os.WriteFile(filepath.Join(d, "nodes.json"), []byte(data), 0644); err != nil {
		t.Fatal(err)
	}
	...
	if len(ns) != 1 || ns[0].ID != "india" {
		t.Fatalf("unexpected nodes: %+v", ns)
	}

	n, err := Find(d, "india")
	if err != nil || n.Country != "India" {
		t.Fatalf("find failed: %+v %v", n, err)
	}
	if _, err := Find(d, "missing"); err == nil {

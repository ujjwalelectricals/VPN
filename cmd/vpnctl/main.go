...
func main() {
	...
	case "list":
		cmdList(root)
	case "connect":
		requireArg(os.Args, 2, "vpnctl connect <node-id>")
		cmdConnect(root, os.Args[2])
	case "disconnect":
		requireArg(os.Args, 2, "vpnctl disconnect <node-id>")
		cmdDisconnect(root, os.Args[2])
	case "status":
		requireArg(os.Args, 2, "vpnctl status <node-id>")
	...
	case "dashboard":
		cmdDashboard(root)
	case "version":
		fmt.Println("Ujjwal FreeVPN 0.1.0")
	default:
		usage()
	}
}

func requireArg(args []string, index int, usage string) {
	if len(args) <= index {
		fatal(usage)
	}
}
...
func cmdDisconnect(root, id string) {
	...
	if err := wireguard.Stop(n); err != nil {
		fatal(err.Error())
	}
...
func cmdDoctor(root string) {
	report := map[string]any{
		...
		"security_note":      "Real private keys must remain local and must never be committed to GitHub.",
		"configuration_note": "A node needs a valid client profile and reachable WireGuard server.",
	}
	printJSON(report)
}
